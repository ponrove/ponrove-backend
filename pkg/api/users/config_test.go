...
// TestRequiredConfiguration checks that the server package loads the configuration required by the users API.
func TestRequiredConfiguration(t *testing.T) {
	t.Parallel()

	cfg := config.New()
	err := users.Register(cfg, humachi.New(chi.NewRouter(), huma.DefaultConfig("", "")))
	assert.NoError(t, err, "failed to register users API")

	cleanCfg := configura.NewConfigImpl()
	err = users.Register(cleanCfg, humachi.New(chi.NewRouter(), huma.DefaultConfig("", "")))
