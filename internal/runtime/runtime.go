...
func Runtime(ctx context.Context, cfg configura.Config, bundles ...api.APIBundle) error {
	...
	if err := api.RegisterAPIBundles(cfg, h, bundles...); err != nil {
	...
	shutdownTimeout := time.Duration(cfg.Int64(config.SERVER_SHUTDOWN_TIMEOUT)) * time.Second
